...
SILRAD is valuable for a separate, SILRAD-native experiment. The included
`fasttext-all-nofamily.csv` is a numerical FastText representation rather than
raw Sysmon XML with preserved process/path/timestamp fields. It is therefore
not mixed with BRDS raw-Sysmon windows for performance claims. Genuine local
benign `.evtx`/XML captures remain required for the BRDS raw-Sysmon baseline.
...
### Step 1: SILRAD-native evaluation
- Preserve the authors' train/test or execution/family boundaries.
- Train and evaluate using the native FastText representation only.
- Do not claim that its proxy windows are equivalent to raw five-second Sysmon process windows.

### Step 2: External raw-Sysmon evaluation
- Collect genuine benign Sysmon `.evtx`/XML telemetry and process it through `run_pipeline.py`.
- Use Splunk raw Sysmon logs as a separately labelled attack source after schema alignment.
